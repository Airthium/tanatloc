      ...
      var keywordMapper = this.createKeywordMapper(
        {
          'variable.language':
            ...
            'this|arguments|prototype|window|document', // Pseudo
          keyword:
            'int|real|varf|problem|yield|import|get|set|async|await|' +
            'break|case|catch|continue|default|delete|do|else|finally|for|function|' +
            'if|in|of|instanceof|new|return|switch|throw|try|typeof|let|var|while|with|debugger|' +
            '__parent__|__count__|escape|unescape|with|__proto__|' +
            'class|enum|extends|super|export|implements|private|public|interface|package|protected|static',
...
ace.define(
  ...
  function (require, exports, module) {
    ...
    ;(function () {
      ...
      this.getPropertyValueCompletions = function (
        state,
        session,
        pos,
        prefix
          ...
          if (text == '\n') {
            ...
            if (token && token.type.indexOf('tag-close') !== -1) {
              ...
              if (this.voidElements && !this.voidElements[tag]) {
                var nextToken = session.getTokenAt(
                  cursor.row,
                  cursor.column + 1
                )
                var line = session.getLine(row)
...
ace.define(
  ...
  function (require, exports, module) {
    ...
    ;(function () {
      ...
      this.getAttributeValueCompletions = function (
        state,
        session,
        pos,
        prefix
...
ace.define(
  ...
  function (require, exports, module) {
    ...
    var HtmlHighlightRules =
      require('./html_highlight_rules').HtmlHighlightRules
  ...
  function (require, exports, module) {
    ...
    ;(function () {
        ...
        if (isStart && !isEnd) {
          ...
          if (match[1]) {
            if (
              ...
              if (
                /^\s*(?:if|else|while|until|unless)\s*/.test(line) === false
              ) {
                return
              }
            }
            ...
            if (
              session.getTokenAt(row, match.index + 1).type ===
              'comment.multiline'
            )
              return 'start'
          } else {
      ...
      this.rubyBlock = function (session, row, column, tokenRange) {
        ...
        if (
          !token ||
          (token.type != 'keyword' && token.type != 'comment.multiline')
        )
          return
            ...
            switch (token.value) {
              case 'do':
                for (var i = stream.$tokenIndex - 1; i >= 0; i--) {
                  ...
                  if (
                    prevToken &&
                    (prevToken.value == 'while' ||
                      prevToken.value == 'until' ||
                      prevToken.value == 'for')
                  ) {
                    level = 0
                    break
...
ace.define(
  ...
  function (require, exports, module) {
    ...
    var RubyHighlightRules =
      require('./ruby_highlight_rules').RubyHighlightRules
    var MatchingBraceOutdent =
      require('./matching_brace_outdent').MatchingBraceOutdent
    var Range = require('../range').Range
    var CstyleBehaviour = require('./behaviour/cstyle').CstyleBehaviour
    ...
    ;(function () {
      ...
      this.checkOutdent = function (state, line, input) {
        return (
          /^\s+(end|else|rescue|ensure)$/.test(line + input) ||
          this.$outdent.checkOutdent(line, input)
        )
      }
