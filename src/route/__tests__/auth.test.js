...
describe('route/auth', () => {
  ...
  test('authorized', async () => {
    ...
    res = await auth({ id: 'id' }, { owners: ['id'] })
    expect(res).toBe(true)

    res = await auth({ id: 'id' }, {}, { owners: ['id'] })
    expect(res).toBe(true)

    res = await auth({ id: 'id' }, { owners: ['id'], users: ['id'] })
    expect(res).toBe(true)

    res = await auth({ id: 'id' }, { users: ['id'] })
    expect(res).toBe(true)
    ...
    res = await auth({ id: 'id' }, { groups: ['id'] }, {})
    expect(res).toBe(true)
  ...
  test('not authorized', async () => {
    ...
    res = await auth({ id: 'id' }, { owners: ['id2'] })
    expect(res).toBe(false)

    res = await auth({ id: 'id' }, { owners: ['id2'], users: ['id2'] })
    expect(res).toBe(false)

    res = await auth({ id: 'id' }, { users: ['id2'] })
    expect(res).toBe(false)
