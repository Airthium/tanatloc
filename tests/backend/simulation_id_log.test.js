...
describe('e2e/backend/simulation/id/log', () => {
  const req = {}
  let resStatus
  let resJson
  const res = {
  ...
  test('Get log', async () => {
    ...
    expect(mockCaptureException).toHaveBeenLastCalledWith(
      new Error('Missing data in your request (body: { file(string) })')
    )

    // Create a fake log file
    await Tools.writeFile(path.join(SIMULATION, simulation.id), 'log', 'log')
    ...
    // Error
    jest.spyOn(SimulationLib, 'getLog').mockImplementationOnce(() => {
